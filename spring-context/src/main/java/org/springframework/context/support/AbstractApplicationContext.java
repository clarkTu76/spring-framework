...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected ConfigurableEnvironment createEnvironment() {
		//StandardEnvironment 没有无参的构造器 默认调用的是父类的AbstractEnvironment的无参构造器
		//初始化了AbstractEnvironment 中的许多配置文件中的key
		//然后回到StandardEnvironment customizePropertySources()
	...
	public void refresh() throws BeansException, IllegalStateException {
		//同步监视器 用于同步刷新和销毁操作  super(parent)初始化的
		synchronized (this.startupShutdownMonitor) {
			...
			//创建 DefaultListableBeanFactory 解析xml 创建BeanDefinition 注册到 beanFactory中
			//并创建各种 BeanDefinition 的后置处理器(BeanFactoryPostProcessor)的 BeanDefinition
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			//beanFactory准备工作，对其填充各种属性
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 空方法,子类相关方法拓展,spring自己在web中有拓展
	...
	protected void prepareRefresh() {
		...
		//关闭标志是 false 激活标志是true 表示当前容器正在启动
		...
		// setConfigLocations已经创建了StandardEnvironment  验证需要的属性是否在环境中
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//初始化BeanFactory,并进行xml的读取，并将得到的beanFactory记录在当前this中
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//设置beanFactory的classLoader作为当前context的classLoader  app classLoader
		beanFactory.setBeanClassLoader(getClassLoader());
		//设置beanFactory的表达式处理器
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		//给beanFactory增加一个默认的propertyEditor 在实例化填充属性的时候用
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		//添加BeanPostProcessor,ApplicationContextAwareProcessor此类完成某些Aware对象的注入
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		//接口的实现是由容器通过set方法注入的
		//所以在使用autowire进行注入的时候需要将这些接口忽略
		...
		// 设置几个自动装配的特殊规则,当在进行ioc初始化的如果有多个实现，那么就使用指定的对象进行注入 尤其是在by_type注入的时候
		...
		// 注册默认的系统环境bean(StandardEnvironment)到一级缓存中
	...
	/**
	 * 实例化 排序 并且调用所有已经注册了beanFactoryPostProcessor
	 * Instantiate and invoke all registered BeanFactoryPostProcessor beans,
	 * respecting explicit order if given.
	 * <p>Must be called before singleton instantiation.
	 */
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		//获取到当前应用上下文中的 beanFactoryPostProcessor 变量集合
		//并且实例化调用执行所有已经注册的 beanFactoryPostProcessor
		//默认情况下 getBeanFactoryPostProcessors() 返回的是空的集合
		//这里能识别的 是重写 addBeanFactoryPostProcessor() 的，xml配置的这里获取不到 xml配置的在beanDefinition中
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 冻结所有的 beanDefinition，说明注册的beanDefinition将不被修改或任何进一步的处理
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		//resourcePatternResolver 是 super(parent)时初始化的
